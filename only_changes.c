...
#include <stdio.h>
#include <stdlib.h>
#include <string.h> // Für strchr(), strncpy(), strcmp()
...
// ----------------------------------------------------
// NEU: CACHE FÜR SATELLITEN-ZUSTÄNDE
// ----------------------------------------------------
#define MAX_SATELLITES_IN_VIEW 128 // Max. Satelliten, die wir gleichzeitig verfolgen

typedef struct {
    char system_name[32]; // Z.B. "GPS (USA)"
    char sat_id[32];      // Z.B. "19"
    
    // Die zuletzt geloggten Werte
    char elevation[32];
    char azimut[32];
    char snr[32];
} SatelliteState;

// Statischer Cache, um den letzten Zustand jedes Satelliten zu speichern.
// Globale/Statische Variablen werden automatisch mit 0 initialisiert.
// d.h. alle Strings (system_name, sat_id) sind anfangs leer ("").
static SatelliteState satellite_cache[MAX_SATELLITES_IN_VIEW];
...
// ----------------------------------------------------
// ANGEPASSTE PARSER-FUNKTION (mit Cache-Logik)
// ----------------------------------------------------
    ...
    while (*p) { // Solange der Zeiger nicht am Ende des Strings ('\0') ist
        ...
        if (sat_field_index == 4) {
            ...
            if (strlen(sat_data[0]) > 0) {
                ...
                // --- NEUE LOGIK: Cache prüfen ---
                
                int cache_index = -1;
                int free_slot = -1;
                int has_changed = 0;

                // 1. Suche Satellit im Cache (System + ID)
                for (int i = 0; i < MAX_SATELLITES_IN_VIEW; i++) {
                    // Merke dir den ersten freien Slot, falls wir ihn brauchen
                    if (satellite_cache[i].sat_id[0] == '\0' && free_slot == -1) {
                        free_slot = i;
                    }
                    
                    // Vergleiche System UND ID
                    if (strcmp(satellite_cache[i].system_name, system_name) == 0 &&
                        strcmp(satellite_cache[i].sat_id, id) == 0) {
                        cache_index = i;
                        break;
                    }
                }
                ...
                // 2. Vergleichen
                if (cache_index != -1) {
                    // Satellit gefunden. Prüfe auf Änderungen.
                    if (strcmp(satellite_cache[cache_index].elevation, elev) != 0 ||
                        strcmp(satellite_cache[cache_index].azimut, azimut) != 0 ||
                        strcmp(satellite_cache[cache_index].snr, snr) != 0) {
                        has_changed = 1; // Ja, es hat sich was geändert
                    }
                } else {
                    // Satellit nicht gefunden, ist also NEU
                    has_changed = 1;
                    if (free_slot != -1) {
                        cache_index = free_slot; // Neuen Slot zuweisen
                    } else {
                        // Cache ist voll! (sehr unwahrscheinlich)
                        // Wir überschreiben einfach den ersten Eintrag (simple FIFO)
                        // Bessere Strategien (LRU) wären möglich, aber komplexer.
                        cache_index = 0; 
                    }
                }

                // 3. Nur loggen, WENN neu oder geändert
                if (has_changed) {
                    ...
                    if (cache_index != -1) { // Nur cachen, wenn Platz da war/gemacht wurde
                        strcpy(satellite_cache[cache_index].system_name, system_name);
                        strcpy(satellite_cache[cache_index].sat_id, id);
                        strcpy(satellite_cache[cache_index].elevation, elev);
                        strcpy(satellite_cache[cache_index].azimut, azimut);
                        strcpy(satellite_cache[cache_index].snr, snr);
                    }
                }
                // --- ENDE NEUE LOGIK ---
            }
...
int main() {
    ...
    // 2. Seriellen Port (COM7) öffnen
    HANDLE hSerial;
    hSerial = CreateFile(
        "\\\\.\\COM4", GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0
    );
    if (hSerial == INVALID_HANDLE_VALUE) {
        fprintf(stderr, "Fehler beim Öffnen von COM7. Fehlercode: %lu\n", GetLastError());
        fclose(output_file);
        return 1;
    }
    ...
    printf("Starte ROBUSTEN GSV-Satelliten-Logger auf COM7...\n");
    printf("Schreibe in 'satellite_log.csv'\n(Strg+C zum Beenden)\n\n");
    printf("LOGGE NUR AENDERUNGEN (DE-DUPLIZIERT):\n");
